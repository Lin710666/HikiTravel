...
(function () {
  ...
  function renderBatch() {
    ...
    // 标题原本写成 '**' + title + '**'，靠 renderMarkdown 渲染成粗体。
    // 但外壳并没有把 appendMsg 暴露到 window 上（bubbleHost 会走 fallback 的
    // innerHTML），于是那两个星号就原样显示出来了 —— 页面上能看到
    // 「**第 1 步 · 去哪、去几天**」。改成真正的元素，两个路径都正常，
    // 顺带给了样式一个钩子（.ask-title，按文旅工作台的卡片标题来排）。
    var bd = bubbleHost(
      '<div class="ask-title">' + batch.title +
      '<span class="ask-step">第 ' + (st.bi + 1) + ' / ' + BATCHES.length + ' 批</span></div>'
    );
    if (!bd) return;
    host = bd;
   ...
   */
  function onDone() {
    // 一个气泡都没选：只有**输入框里也没内容**时才拦下来。
    // 需求：气泡不必选完 —— 用户直接在输入框里说清楚了，就该照他说的出方案。
    if (!anyAnswered() && !externalInput()) {
      subtitle('请认真选择，或者直接在下面输入框里说说你的需求');
      return;
    }
    finish();
  }

  /** 输入框里有没有内容（由外部注入，模块本身不该知道输入框在哪） */
  function externalInput() {
    if (!st || typeof st.hasInput !== 'function') return false;
    try { return !!st.hasInput(); } catch (e) { return false; }
  }
  ...
  window.WenlvAsk = {
     ...
     * opts.hasInput —— 返回"输入框里现在有没有内容"。有内容时，气泡一个都不选
     * 也允许直接出方案（需求：以用户选了多少、输入多少为基准）。
     */
    start: function (onDone, opts) {
      var l = logEl();
      if (!l) return false;
      if (st) { subtitle('先把上面那批选完'); return false; }
      // 一批气泡近 400px 高，而默认布局的聊天区可见高度只有 105px ——
      // 不先把输出区撑起来，主人只能看到标题那一行，会以为样式没改。
      if (window.WenlvChatPane && window.WenlvChatPane.autoExpand) {
        try { window.WenlvChatPane.autoExpand(); } catch (e) { /* 忽略 */ }
      }
      st = { bi: 0, answers: {}, onDone: onDone, hasInput: (opts && opts.hasInput) || null };
      renderBatch();
      return true;
    },
