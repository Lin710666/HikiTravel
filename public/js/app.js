...
(function () {
  ...
  function bindPlanner() {
    /* ★ 改成嵌入「组员那版」的规划页（iframe）。
     *
     * 原来是挂 planner-embed.js（融合版自己那套 React 工作台）。需求方看过组员
     * main 上那版之后定了：**用他那张表单页**，原来的工作台不要了。
     *
     * 为什么用 iframe 而不是再打一个 IIFE：
     *   · 他那套是完整的 Vite 单页应用（有路由、有状态、自带样式），不是 lib 模式
     *     的嵌入包 —— 想挂进同一个文档得先把它改造成 UMD，那是改人家的工程。
     *   · iframe 天然隔离：他的 antd 样式不会污染我们的 AIRI 界面，反之亦然。
     *   · 同源（同一个后端托管 /planner/），所以他的 /api/plan 直接打到我们的后端上，
     *     跨域、cookie、路径全都不用管。
     */
    const host = $('#wenlv-planner');
    if (!host) return;

    if (host.dataset.wenlvPlanner === 'iframe') return;   // 只挂一次
    host.dataset.wenlvPlanner = 'iframe';
    host.innerHTML = '';
    ...
    /* ★ 风格统一：他那套是浅色 antd，嵌进我们的深色界面像"贴了块白纸"。
       同源 iframe 能拿到 contentDocument，所以**从外面往里注入**一份深色主题
       （public/css/planner-theme.css）—— 不用改他的工程、也不用重新构建。
       放在 load 之后注入：他那套是 vite 单页应用，样式是运行时挂上去的，
       注入太早会被它自己的 CSS-in-JS 覆盖。 */
    frame.addEventListener('load', () => {
      try {
        const doc = frame.contentDocument;
        if (!doc || doc.getElementById('airi-planner-theme')) return;
        const link = doc.createElement('link');
        link.id = 'airi-planner-theme';
        link.rel = 'stylesheet';
        link.href = '/css/planner-theme.css';
        doc.head.appendChild(link);
      } catch (e) {
        /* 跨域或还没加载好 —— 不影响功能，只是不统一而已，不抛 */
        console.warn('规划页主题注入失败（不影响使用）', e);
      }
    });
