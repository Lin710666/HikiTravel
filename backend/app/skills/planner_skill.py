...
class PlannerSkill(Skill):
    ...
    def _generate_rules(self, ctx: dict[str, Any]) -> TravelPlan:
        ...
        days: List[DailyPlan] = []
        used_restaurants: set = set()     # 同一家餐厅全程只出现一次
        for i in range(pref.duration_days):
            ...
            #
            # `used_restaurants` 是合并两条线时保留的组员那边的好做法：
            # 同一家餐厅全程只出现一次（原来 `i % len(restaurants)` 会绕回去重复）。
            lunch = self._pick(
                restaurants, i, anchors=day_pois[:mid] or day_pois, used=used_restaurants
            )
    ...
    def _pick(
        ...
        """选一家餐厅：优先**当天景点附近**的，且全程不重复。

        合并两条线时这里是"同一件事、两种做法"，两边各解对了一半，所以合起来：

        · **组员那条线**：按序号取、**不取模**，取尽后返回「就近用餐」占位 ——
          保证同一家餐厅不会在行程里重复出现（原来 `i % len` 会绕回去）。
          这个做法保留下来了（`used` 集合 + 取尽给占位）。
        · **我们这条线**：按**当天景点的位置**就近选 —— 原来完全不看位置，
          会排出"在松江广富林玩完、午餐订到人民广场"这种段落，
          中间打车 40 公里、138 元（实测数据）。
          另外午餐锚上午那半、晚餐锚下午那半，见调用处的说明。
        """
        placeholder = POI(
        ...
        pool = [r for r in restaurants if r.name not in (used or set())]
        if not pool:
            return placeholder      # 取尽 → 占位；绝不重复用过的（组员那边的做法）

        anchor = self._anchor_location(anchors or [])
        if anchor is None or not any(r.location.lat or r.location.lng for r in pool):
        ...
        return sorted(enumerate(pool), key=lambda kv: (dist_to(kv[1]), kv[0]))[0][1]
